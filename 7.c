...
double quad_select(int fun_no, int quad_no, double a, double b, int n) {
    double wynik = quad_tab[quad_no](func_tab[fun_no], a, b, n);
    return wynik;
}
...
double recurs(Func1vFp f, double a, double b, double S, double delta, QuadratureFp quad, int level) {
    if (level > RECURS_LEVEL_MAX){
        ...
        double c = (b - a) / 2;
        double s1 = quad(f, a, a + c, 1);
        double s2 = quad(f, a + c, b, 1);
        if ( fabs(s1 + s2 - S) <= delta ){
            return s1 + s2;
        }
        else{
            return recurs(f, a, a + c, s1, delta/2, quad, level + 1) + recurs(f, a + c, b, s2, delta/2, quad, level + 1);
        }
    }
}
...
double dbl_integr_normal_1(Func2vFp f, double x1, double x2, int nx, double hy, Func1vFp fg, Func1vFp fh)  {
    ...
    for ( int i = 0; i < nx; i++){
        double y2 = fh(x1);
        double y1 = fg(x1);
        double ny = ceil(( y2 - y1 ) / hy);
        double dy = ( y2 - y1 ) / ny;
        for ( int j = 0; j < ny; j++){
            wynik += dx * dy * f(x1, y1 + dy/2);
            y1 += dy;
        }
        x1 += dx;
    }
...
double trpl_quad_rect(FuncNvFp f, const double variable_lim[][2], const int tn[], BoundNvFp boundary) {
    ...
    double tab[3];
    double a, b;
    tab[0] = variable_lim[0][0];
    tab[1] = variable_lim[1][0];
    tab[2] = variable_lim[2][0];
    a = variable_lim[1][0];
    b = variable_lim[2][0];
...
void recur_quad_rect_mid(double *psum, FuncNvFp f, int variable_no, double tvariable[], const double variable_lim[][2], const int tn[], int level, BoundNvFp boundary) {
    if (level == variable_no){
        if (boundary == NULL || boundary(tvariable, variable_no)){
            double wynik = f(tvariable, variable_no);
            for (int i = 0; i < variable_no; i++){
                double d = (variable_lim[i][1] - variable_lim[i][0]) / tn[i];
                wynik *= d;
            }
            *psum += wynik;
        }
...
int main(void) {
    ...
    switch (to_do) {
        ...
        case 7: // 7.3.3 calka n-wymiarowa na hiperprostopadloscianie
            ...
            for (int i = 0; i < n; i++){
                double d = (variable_lim[i][1] - variable_lim[i][0])/tn[i];
                tvariable[i] = variable_lim[i][0] + d/2;
            }
