...
void aver_varian(const double tab[], size_t n, double *arith_average, double *variance) {
    ...
    *(arith_average) = sum_value / n;
    double sum_squares = 0;
    for (int i = 0; i < n; i++){
        sum_squares += (tab[i] - *(arith_average)) * (tab[i] - *(arith_average));
    }
...
void bernoulli_gen(int tab[], size_t n, double probab) {
    ...
    for(int i = 0; i < n; i++){
        wynik = rand();
        if(wynik - probab < probab * (RAND_MAX)){
            tab[i] = 1;
        }
        else tab[i] = 0;
    }
...
void print_histogram(const double tab[], size_t n, int x_start, double y_scale, char mark) {
    for(int i = x_start; i < n + x_start; i++){
        ...
        printf("|");
        for(int j = 0; j < round(tab[i-2] / y_scale); j++){
            printf("%c", mark);
        }
        printf(" %.3f\n", tab[i-2]);
    }
